...
fn main() {
    ...
    println!("   Is zero address: {}", SqlAddress::ZERO.to_string().starts_with("0x000000000"));
    ...
    println!("   All methods equal: {}", 
        SqlAddress::ZERO == zero_from_str && 
        SqlAddress::ZERO == zero_from_macro
    );
    ...
    for (i, addr) in test_addresses.iter().enumerate() {
        println!("   Address {}: {} -> Is zero: {}, Valid for transfer: {}", 
            i, addr, is_zero_address(*addr), is_valid_transfer_address(*addr)
        );
    }
    ...
    for (i, addr) in address_registry.iter().enumerate() {
        let status = if *addr == SqlAddress::ZERO { "Placeholder" } else { "Active" };
    ...
    for user in &users {
        println!("     User #{}: {} - Wallet: {} - Status: {}", 
            user.id, user.name, user.wallet_address, user.get_status());
    }
    
    // Demonstrate filtering and business logic
    let users_needing_setup: Vec<_> = users.iter()
        .filter(|user| !user.has_wallet())
        .collect();
    
    let eligible_for_referral_bonus: Vec<_> = users.iter()
        .filter(|user| user.has_wallet() && user.has_referrer())
        .collect();
    
    println!("\n   Business Analytics:");
    println!("     Users needing wallet setup: {}", users_needing_setup.len());
    println!("     Users eligible for referral bonus: {}", eligible_for_referral_bonus.len());
    
    // Smart contract interaction simulation
    println!("\n7. Smart Contract Integration Patterns:");
    
    /// Simulates a token contract interaction
    fn simulate_token_transfer(from: SqlAddress, to: SqlAddress, amount: u64) -> Result<String, String> {
        if from == SqlAddress::ZERO {
            return Err("Cannot transfer from zero address (minting requires special permissions)".to_string());
        }
    ...
    let transfer_scenarios = [
        (sqladdress!("0x742d35Cc6635C0532925a3b8D42cC72b5c2A9A1d"), SqlAddress::ZERO, 100), // Burn
        (SqlAddress::ZERO, sqladdress!("0x742d35Cc6635C0532925a3b8D42cC72b5c2A9A1d"), 50), // Invalid mint
        (sqladdress!("0x742d35Cc6635C0532925a3b8D42cC72b5c2A9A1d"), 
         sqladdress!("0x1234567890123456789012345678901234567890"), 75), // Normal transfer
    ];
