...
fn get_tx_info() -> TxInfo {
    // Example function to create a TxInfo instance
    TxInfo {
        id: None, // ID is optional and can be set later
...
async fn test_mysql_operations() {
    ...
    sqlx::query("INSERT INTO tx_infos (tx_hash, from_address, to_address, value, data, gas, gas_price, topic) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    ...
    let tx: TxInfo = sqlx::query_as("SELECT * FROM tx_infos WHERE from_address = ?")
        .bind(&tx_info.from_address)
        .fetch_one(&pool)
        .await
