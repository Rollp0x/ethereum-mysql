...
fn main() {
    ...
    println!(
        "  Balance doubled (old): balance * SqlU256::from(2u64) = {}",
        doubled_old
    );
    ...
    println!(
        "  Transaction cost (new): gas_price * {} = {} wei",
        gas_limit, tx_cost_new
    );
    println!(
        "  Transaction cost (old): gas_price * SqlU256::from({}) = {} wei",
        gas_limit, tx_cost_old
    );
