...
mod sqlite_tests {
    ...
    async fn test_sqlite_zero_address() {
        ...
        assert_eq!(retrieved_addr.to_string(), "0x0000000000000000000000000000000000000000");
    }
    ...
    async fn test_sqlite_macro_addresses() {
        ...
        let addresses = [
            (sqladdress!("0x742d35Cc6635C0532925a3b8D42cC72b5c2A9A1d"), "Regular Address"),
            (sqladdress!("0x0000000000000000000000000000000000000000"), "Zero Address"),
            (sqladdress!("dEaD000000000000000000000000000000000000"), "Dead Address"),
        ];
    ...
    async fn test_sqlite_multiple_addresses() {
        ...
        for (user_id, addr_str, is_primary) in &test_data {
            ...
            sqlx::query(
                "INSERT INTO wallet_addresses (user_id, wallet_address, is_primary) VALUES (?, ?, ?)"
            )
            .bind(user_id)
            .bind(&addr)
            .bind(is_primary)
        ...
        let primary_wallets = sqlx::query(
            "SELECT user_id, wallet_address FROM wallet_addresses WHERE is_primary = TRUE ORDER BY user_id"
        )
        .fetch_all(&pool)
        .await
        ...
        for (i, row) in primary_wallets.iter().enumerate() {
            ...
            match i {
                0 => assert_eq!(addr.to_string(), "0x742D35CC6635C0532925A3b8D42cC72b5c2A9a1D"),
                1 => assert_eq!(addr, SqlAddress::ZERO),
                2 => assert_eq!(addr.to_string(), "0xdEad000000000000000000000000000000000000"), // Note lowercase 'a'
...
mod mysql_tests {
    ...
    async fn setup_mysql_test() -> Option<MySqlPool> {
        ...
        match MySqlPool::connect(&database_url).await {
            Ok(pool) => {
                ...
                .is_ok() {
                    // Clean existing data
                    let _ = sqlx::query("DELETE FROM ethereum_addresses").execute(&pool).await;
                    Some(pool)
                } else {
                    None
                }
    ...
    async fn test_mysql_zero_and_special_addresses() {
        ...
        let special_addresses = [
            ...
            (sqladdress!("0xffffffffffffffffffffffffffffffffffffffff"), "Max Address"),
            (sqladdress!("0xdead000000000000000000000000000000000000"), "Dead Address"),
        ];
...
mod postgres_tests {
    ...
    async fn test_postgres_basic_operations() {
        ...
        // Test querying address
        let row = sqlx::query(&format!("SELECT address, label FROM {} WHERE id = 1", table_name))
            .fetch_one(&pool)
            .await
            .expect("Failed to select address");
    ...
    async fn test_postgres_transaction_operations() {
        ...
        let test_addresses = [
            (sqladdress!("0x1111111111111111111111111111111111111111"), "Address 1"),
            (sqladdress!("0x2222222222222222222222222222222222222222"), "Address 2"),
            (sqladdress!("0x3333333333333333333333333333333333333333"), "Address 3"),
        ];
    ...
    async fn test_postgres_advanced_queries() {
        ...
        let wallet_data = [
            (1, sqladdress!("0x742d35Cc6635C0532925a3b8D42cC72b5c2A9A1d"), "Primary Wallet", true),
            (1, sqladdress!("0x1234567890123456789012345678901234567890"), "Secondary Wallet", false),
            (2, SqlAddress::ZERO, "Empty Wallet", true),
            (3, sqladdress!("0xdead000000000000000000000000000000000000"), "Burn Address", true),
        ];
